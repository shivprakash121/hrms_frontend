...
import { getUserDataAction, postProfileUploadAction } from "../store/action/userDataAction";
  ...
  return (
    <div className="w-full p-4 sm:p-6">
      ...
      <div className="border-b border-gray-200 p-4 sm:p-6 flex items-center space-x-4">
        {loading ? (
          ...
          <div className="relative">
            <img
              src={user || userDataList?.employeePhoto || selectedImage}
              alt="Profile"
            ...
            {/* <img
              src={editText}
              alt="Profile"
              className="absolute w-8 h-8 bottom-0 right-0 text-white p-1 rounded-full cursor-pointer object-cover"
            />
            <input
              id="avatarUpload"
              type="file"
      ...
      <div className="border-b border-gray-200 p-4 sm:p-6">
        <h2 className="text-sm sm:text-md font-bold text-gray-800 mb-4">
          ...
          {loading
            ...
            : [
              ...
              <div key={index} className="flex-1 min-w-[45%]">
                <p className="text-xs sm:text-sm text-gray-900 font-semibold  sm:tracking-tight">{item.label}</p>
                <p className="text-sm sm:text-md text-gray-500">{item.value}</p>
              </div>
            ))}
        </div>
      </div>
      ...
      <div className="p-4 sm:p-6">
        <h2 className="text-sm sm:text-md font-bold text-gray-800 mb-4">Address</h2>
          ...
          {loading ? (
            ...
            <>
              {[
                ...
                <div key={index}>
                  <p className="text-xs sm:text-sm text-gray-500 font-semibold">{item.label}</p>
