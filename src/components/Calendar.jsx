...
function Calendar({ employeeId, userRole }) {
  ...
  const { data: leaveData, error: newError } = useSelector(
    (state) => state.compoffReducer
  );
  ...
  const handleSubmit = (e) => {
    ...
    if (leaveType === "compOff") {
      ...
      if (!selectDuration) {
        alert("Please select duration for Vendor Meeting");
        return;
      }
      ...
      dispatch(
        postVendorMeetingAction({ leaveType, leaveStartDate, reason, duration: selectDuration })
      );
      setModalOpen(false);
      return;
    } else {
  ...
  return (
    <>
      <div className="flex gap-2 mb-2 text-xs sm:text-sm md:text-base">
        <h3>Leave Define: </h3>
        <span>
          EL: (Earned Leave) / CL: (Casual Leave) / OL: (Optional Leave) / ML:
          (Medical Leave) / Reg-L: (Regularized) / SL: (Short Leave) /{" "}
        </span>
      </div>
      <div className="flex gap-2 mb-2 text-xs sm:text-sm md:text-base">
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
            <div className="bg-white rounded-lg p-6 w-11/12 sm:w-2/3 md:w-1/2 lg:w-1/3">
              <h2 className="text-sm sm:text-base md:text-lg font-semibold mb-4">
                Selected Date: {selectedDay} {months[currentMonth]}{" "}
                {currentYear}
              </h2>
              <div className="mb-4">
                <label
                ...
                {/* Vendor Meeting Duration Dropdown */}
                {selectType === "vendor-meeting" && (
                  <div className="mb-4 mt-4">
                    <label
                      htmlFor="vendorDuration"
                      className="block text-xs sm:text-sm md:text-base font-medium text-gray-700"
                    >
                      Select Duration (Vendor Meeting)
                ...
                {/* Comp-Off Duration Dropdown */}
                {selectType === "compOff" && (
                  <div className="mb-4 mt-4">
                    <label
                      htmlFor="compOffDuration"
                      className="block text-xs sm:text-sm md:text-base font-medium text-gray-700"
                    >
                      Select Duration (Comp-Off)
                      <span className="text-red-500">*</span>
                    </label>
                    <select
                      id="compOffDuration"
                      name="compOffDuration"
