...
const CreateProjectModal = ({ tittleBtn, onClick }) => {
  ...
  const { loading: uploadLoading, data: medicalReport } = useSelector(
    (state) => state.medicalFileReducer
  );
  const navigate = useNavigate();
  const { data: dataa, error } = useSelector(
    (state) => state.leaveApplyByEmployee
  );
  ...
  useEffect(() => {
    if (medicalReport && medicalReport.location) {
      // ✅ Ensure medicalReport is defined
      ...
      case "casualLeave":
        ...
        if (startDate < currentDate || startDate > sevenDaysFromNow) {
          setLeaveError((prevErrors) => ({
            ...prevErrors,
            casual:
              "Casual leave can only be applied for dates between today and the next 7 days.", // Update the `casual` field
          }));
          return;
        }
      ...
      case "medicalLeave":
        if (!(startDate <= yesterday && startDate >= thirtyDaysAgo)) {
          setLeaveError((prevErrors) => ({
            ...prevErrors,
            medical:
              "Medical leave can only be applied for dates between yesterday and the last 30 days.",
          }));
          return;
        }
        ...
        if (leaveData.totalDays < 1 || leaveData.totalDays > 7) {
          setLeaveError((prevErrors) => ({
            ...prevErrors,
            medical:
              "Medical leave must be applied for a minimum of 1 day and a maximum of 7 days.",
          }));
          return;
        }
      ...
      case "earnedLeave":
        ...
        if (startDate < firstDayOfMonth || startDate > lastDayOfMonth) {
          setLeaveError((prevErrors) => ({
            ...prevErrors,
            earned:
              "Earned leave can only be applied within the current month.",
          }));
          return;
        }
        ...
        if (startDate < minEarnedLeaveDate || startDate > maxEarnedLeaveDate) {
          setLeaveError((prevErrors) => ({
            ...prevErrors,
            earned:
              "Earned leave can only be applied for dates within 14 days before or after today.",
          }));
          return;
        }
        ...
        if (leaveData.totalDays < 1 || leaveData.totalDays > 14) {
          setLeaveError((prevErrors) => ({
            ...prevErrors,
            earned:
              "Earned leave must be applied for a minimum of 1 day and a maximum of 14 days.",
          }));
          return;
        }
  ...
  const getMinDateForLeaveType = () => {
    ...
    switch (leaveData.leaveType) {
      case "casualLeave":
        return formatDate(
          minEarnedLeaveDate > firstDayOfMonth
            ? minEarnedLeaveDate
            : firstDayOfMonth
      ...
      case "medicalLeave":
        return formatDate(
          minEarnedLeaveDate > firstDayOfMonth
            ? minEarnedLeaveDate
            : firstDayOfMonth
        ); // Allow from 31 days ago of current month
      case "earnedLeave":
        return formatDate(
          minEarnedLeaveDate > firstDayOfMonth
            ? minEarnedLeaveDate
  ...
  const getMaxDateForLeaveType = () => {
    ...
    switch (leaveData.leaveType) {
      ...
      case "earnedLeave":
        return formatDate(
          maxEarnedLeaveDate < lastDayOfMonth
            ? maxEarnedLeaveDate
            : lastDayOfMonth
        );
      default:
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg relative z-10 p-6">
            ...
            <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
                ...
                >
                  ...
                  {data?.data?.isNotice === true &&
                    (leaveBalance?.earnedLeave === "0" ? (
                      <option disabled>
                        Earned Leave / {leaveBalance?.earnedLeave}
                      </option>
                    ) : (
                      <option value="earnedLeave">
                        Earned Leave / {leaveBalance?.earnedLeave}
                      </option>
                    ))}
                  ...
                  {data?.data?.isProbation === true &&
                    (leaveBalance?.compOffLeave === "0" ? (
                      <option disabled>
                        Casual Leave / {leaveBalance?.compOffLeave}
                      </option>
                    ) : (
                      <option value="compOffLeave">
                        Casual Leave / {leaveBalance?.compOffLeave}
                      </option>
                    ))}
                  ...
                  {data?.data?.isWorking === true && (
                    <>
                      ...
                      {leaveBalance?.compOffLeave === "0" ? (
                        <option disabled>
                          Comp Off / {leaveBalance?.compOffLeave}
                        </option>
                      ) : (
                        <option value="compOffLeave">
                          Comp Off / {leaveBalance?.compOffLeave}
                      ...
                      {leaveBalance?.medicalLeave === "0" ? (
                        <option disabled>
                          Medical Leave / {leaveBalance?.medicalLeave}
                        </option>
                      ) : (
                        <option value="medicalLeave">
                          Medical Leave / {leaveBalance?.medicalLeave}
                        </option>
                      )}
              ...
              {leaveData.totalDays < 2 &&
              (leaveData.leaveType === "casualLeave" ||
                leaveData.leaveType === "earnedLeave" ||
              ...
              <p class="text-red-600 mt-2">
                {leaveError?.medical ? leaveError.medical : ""}
              </p>
